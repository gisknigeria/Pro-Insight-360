...
const SKILL_LEVELS: Record<SkillLevel, { label: string; score: number; color: string }> = {
  ...
  advanced: { label: "Advanced", score: 75, color: "bg-amber-100 text-blue-800" },
...
export default function TechnicalSkillsForm({ onSubmit, initialData }: TechnicalSkillsFormProps) {
  ...
  const getScoreColor = (score: number) => {
    ...
    if (score >= 50) return "bg-amber-100 text-blue-800 border-blue-300";
      ...
      {results && (
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 border border-amber-200 rounded-lg p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="text-sm text-amber-800 mb-1">Overall Skills Score</div>
          ...
          <div key={category.id} className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                <span className="w-8 h-8 bg-amber-100 text-primary rounded-full flex items-center justify-center text-sm font-bold">
